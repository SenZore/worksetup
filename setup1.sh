...
echo "  +------------------------------------------------------+"
echo ""
echo "  Dual Adapter Layout:"
echo "    Adapter 1 (enp0s3) -> Host-Only  -> DHCP Server"
echo "    Adapter 2 (enp0s8) -> NAT        -> Internet Access"
echo ""
echo "  This script will:"
echo "  1. Wipe ALL previous SSH & DHCP configs"
echo "  2. Fix apt sources (remove cdrom, add online mirrors)"
echo "  3. Install everything fresh"
echo "  4. Configure static IP on enp0s3, DHCP client on enp0s8"
echo "  5. Configure & start DHCP + SSH (auto-start on boot)"
echo ""
...
rm -rf /etc/ssh/sshd_config /etc/ssh/sshd_config.bak* /etc/ssh/sshd_config.original* /etc/motd
rm -rf /etc/dhcp/dhcpd.conf /etc/dhcp/dhcpd.conf.bak* /etc/default/isc-dhcp-server
rm -rf /var/lib/dhcp/dhcpd.leases /var/lib/dhcp/dhcpd.leases~
mkdir -p /var/lib/dhcp && touch /var/lib/dhcp/dhcpd.leases
rm -f /etc/apt/sources.list.original.bak* /etc/apt/sources.list.bak*
...
systemctl is-active --quiet ssh \
  && ok "ssh             -> RUNNING [OK]  (auto-start: enabled)" \
...
systemctl is-active --quiet isc-dhcp-server \
  && ok "isc-dhcp-server -> RUNNING [OK]  (auto-start: enabled)" \
...
ok "networking      -> ENABLED [OK]  (auto-start: enabled)"
# ============================================================
...
echo -e "  ${G}[OK]${N} networking       (ifupdown - applies static IP)"
echo -e "  ${G}[OK]${N} ssh              (OpenSSH server)"
echo -e "  ${G}[OK]${N} isc-dhcp-server  (DHCP server on $IF_HOSTONLY)"
echo ""
